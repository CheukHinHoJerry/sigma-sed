...
class PixelSegmenter(object):
    def __init__(
        self,
        latent: np.ndarray,
        dataset: Union[SEMDataset, TEMDataset, IMAGEDataset, SEDDataset],
        ...
        self.dataset_norm = self.dataset.base_dataset.data #dataset.normalised_elemental_data
        self.method = method
        self.method_args = method_args
        self.height = self.dataset_norm.shape[0] #self.dataset_norm.shape[0]
        self.width = self.dataset_norm.shape[1] #self.dataset_norm.shape[1]

        # Set spectra and nav_img signal to the corresponding ones
        if type(dataset) == SEDDataset:
